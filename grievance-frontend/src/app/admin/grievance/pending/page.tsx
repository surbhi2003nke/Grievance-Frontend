...
const Pending = () => {
  ...
  const fetchStudentInfo = async (rollno: string): Promise<StudentInfo> => {
    ...
    if (!res.ok)
      throw new Error(`Failed to fetch student info for rollno: ${rollno}`);
    const data = await res.json();
  ...
  const fetchAcademicInfo = async (rollno: string): Promise<AcademicInfo> => {
    ...
    if (!res.ok)
      throw new Error(`Failed to fetch academic info for rollno: ${rollno}`);
    const data = await res.json();
              ...
              >
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <button
                    onClick={() =>
                      setExpanded(
                        expanded === grievance.rollno ? null : grievance.rollno
                      )
                    }
